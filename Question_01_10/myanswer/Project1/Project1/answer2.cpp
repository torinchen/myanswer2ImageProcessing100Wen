...
int main(void) {
	...
	for (int i = 0; i < image.rows; i++)
	{
		for (int j = 0; j < image.cols; j++)
		{
			...
			dst1.at<uchar>(i,j) = 0.2126*(float)image_new[2].at<uchar>(i, j) + 0.7152*(float)image_new[1].at<uchar>(i, j) + 0.0722*(float)image_new[0].at<uchar>(i, j);
		}
